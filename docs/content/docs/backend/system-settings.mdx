...
| `billing` | object | 计费入口公开配置 |

`modelChannel` 字段：

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `availableModels` | string[] | 系统可用模型公开名称；保存设置时会自动合并所有已启用私有渠道的 `displayName || model` |
| `modelCosts` | object[] | 模型算力点配置，按公开模型名配置和扣除；后端模型接口调用前预扣，上游失败时返还；未配置默认不扣除 |
...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `rechargeUrl` | string | 前台兑换码弹窗和算力点记录弹窗中的购买按钮跳转地址；为空时隐藏 |
  ...
  "channels": [
    {
      ...
      "modelAliases": [
        { "model": "gpt-image-2", "displayName": "默认图片模型" }
      ],
      "weight": 1,
      "enabled": true,
      "remark": ""
...
| `apiKey` | string | 渠道密钥 |
| `models` | string[] | 该渠道可用的真实上游模型名 |
| `modelAliases` | object[] | 真实模型名到前台完整显示名称的映射；公开列表、默认模型和算力点价格都使用 `displayName || model` |
...
后端调用模型时，会从已启用、已配置 `baseUrl` 和 `apiKey`、且公开模型名或真实模型名匹配的渠道中选择一个，并在请求上游前还原成真实模型名。
